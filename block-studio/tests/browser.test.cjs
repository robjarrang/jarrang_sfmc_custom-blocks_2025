...
async function main() {
  ...
  try {
    ...
    assert.ok(count >= 4, 'Expected detected fields');
    await page.screenshot({
      path: path.join(scratch, 'mapping.png'),
      fullPage: true,
    });
    const heading = (await page.evaluate(() => BlockStudio.getProject().modules[0])).fields.find(
      (f) => f.label === 'Heading',
    );
    assert.ok(heading);
    const mapping = page.frameLocator('#mapping-preview');
    await mapping.locator('h1').click();
    assert.equal(
      await mapping.locator('h1').evaluate((e) => getComputedStyle(e).fontSize),
      '42px',
      'Desktop must not activate the mobile breakpoint',
    );
    await page.waitForFunction(
      () => document.getElementById('selected-field-name').textContent === 'Heading',
    );
    ...
    assert.ok(
      (await page.locator('#trial-source').textContent()).includes(
    ...
    await page.waitForFunction(
      () =>
        Math.round(document.getElementById('trial-preview').getBoundingClientRect().width) === 375,
    );
    assert.equal(
      await page
        .frameLocator('#trial-preview')
        .locator('h1')
        .evaluate((e) => getComputedStyle(e).fontSize),
      '32px',
      'Mobile must activate the responsive CSS',
    );
    await page.locator('#review-export').click();
    await page.locator('#pages-url').fill('https://example.github.io/blocks/');
    await page.locator('#export-location').selectOption('docs');
    await page.locator('#acknowledge-review').check();
    assert.equal(
      await page.locator('#endpoint-value').textContent(),
      'https://example.github.io/blocks/product-story/',
    );
    const [download] = await Promise.all([
      page.waitForEvent('download'),
      page.locator('#download-module').click(),
    ]);
    const zipPath = path.join(scratch, 'module.zip');
    await download.saveAs(zipPath);
    const zip = await JSZip.loadAsync(await fs.readFile(zipPath));
    for (const name of [
      'docs/.nojekyll',
      'docs/product-story/index.html',
      'docs/product-story/icon.png',
      'docs/product-story/dragIcon.png',
      'docs/shared-assets/block-studio-1.6.0/blocksdk.js',
      'PUBLISHING.md',
      'block-studio.jarrang.json',
    ])
      assert.ok(zip.file(name), name);
    assert.ok(!Object.keys(zip.files).some((n) => n.includes('.github/workflows')));
    const definition = JSON.parse(await zip.file('block-studio.jarrang.json').async('string'))
      .modules[0];
    assert.ok(definition.source.includes('Good things start'));
    assert.ok(!definition.source.includes('A fresh start'));
    const [projectDownload] = await Promise.all([
      page.waitForEvent('download'),
      page.locator('#save-project').click(),
    ]);
    await projectDownload.saveAs(path.join(scratch, 'project.json'));
    await page.reload();
    assert.equal(
      (await page.evaluate(() => BlockStudio.getProject().modules[0])).fields.find(
        (f) => f.id === heading.id,
      ).label,
      'Headline',
    );
    await page.locator('#duplicate-module').click();
    assert.equal(await page.locator('.module-button').count(), 2);
    await page.locator('#project-file').setInputFiles(path.join(scratch, 'project.json'));
    await page.locator('#dialog-confirm').click();
    assert.equal(await page.locator('.module-button').count(), 1);
    await page.setViewportSize({ width: 390, height: 844 });
    assert.ok(
      await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth),
      'Studio should fit narrow viewport',
    );
    await page.screenshot({
    ...
    console.log(
      'PASS: import, visual selection, field editing, preview, branch ZIP, project save/reopen, narrow viewport',
    );
    ...
    const origin = 'https://mc.test.exacttarget.com',
      moduleOrigin = 'https://blocks.test';
    ...
    await host.route(moduleOrigin + '/**', async (route) => {
      ...
      let name = 'docs/' + url.pathname.replace(/^\/repository\//, '');
      if (name.endsWith('/')) name += 'index.html';
      const file = zip.file(name);
      if (!file) return route.fulfill({ status: 404, body: 'Not found' });
      let body = await file.async('nodebuffer');
      if (url.searchParams.has('version'))
        body = Buffer.from(body.toString().replace('"release":"1.0.0"', '"release":"2.0.0"'));
      await route.fulfill({
        body,
    ...
    await frame.locator('#' + heading.id).fill('Saved headline');
    const bodyField = definition.fields.find((f) => f.label === 'Text');
    await frame.locator('#' + bodyField.id).fill('NEW EYEBROW');
    await host.waitForFunction(
      ({ h, b }) =>
        window.metadata.studio.values[h] === 'Saved headline' &&
        window.metadata.studio.values[b] === 'NEW EYEBROW',
      { h: heading.id, b: bodyField.id },
    );
    ...
    await frame.locator('#' + heading.id).waitFor();
    assert.equal(await frame.locator('#' + heading.id).inputValue(), 'Saved headline');
    assert.equal(
      await host.evaluate(() => writes.length),
      0,
      'Reopening must not regenerate content',
    );
    ...
    await host.evaluate(() =>
      document
        ...
        .contentWindow.postMessage(
          { method: 'closeBlock', origin: location.origin },
          'https://blocks.test',
        ),
    );
    await host.waitForFunction(() => window.closeAcknowledged);
    assert.equal(
      await host.evaluate((id) => metadata.studio.values[id], heading.id),
    ...
    await frame.locator('#status').filter({ hasText: 'another module version' }).waitFor();
    assert.equal(await host.evaluate(() => writes.length), 0);
    console.log(
      'PASS: actual SDK handshake, rapid edits, metadata preservation, reopen, close flush, version rejection',
    );
